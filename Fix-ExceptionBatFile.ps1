...
class Mapping {
    ...
    [string]$DriveLetter
    [string]$TargetShare
    [String]$NewTargetShare
    Mapping($ScriptPath,$DriveLetter,$TargetShare,$NewTargetShare) {
        $this.ScriptPath=$ScriptPath
        $this.DriveLetter=$DriveLetter
        $this.TargetShare=$TargetShare
        $this.NewtargetShare=$NewTargetShare
    }
}
...
$shareMapping=import-csv -path $shareMappingCSV -Encoding Default
...
$filesToProcess+=[ScriptFile]::New("E:\OtherScripts\LogonScriptUpdate\LogonScripts\Exceptions-W10.bat")
    ...
    try {    
        ...
        if ($null -ne $matchesForFile) {
            ...
            foreach($pattern in $patterns) {                
                ...
                $changedContent=$content | % {
                    if ($_ -match $pattern.pattern) {
                        $impactedMappedDrives+=[Mapping]::New($settingsBatFile.FullName,$matches[1],$matches[2],$Pattern.ReplacementText)
            ...
            if ($file.CountChanges -gt 0) {
                ...
                $backupFileName=$settingsBatFile.Directory.FullName + "\Exceptions-W10" + $changeID + "_" + (get-date).ToString('yyyy-MM-dd-HHmmss') + ".bat"
